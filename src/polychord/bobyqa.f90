...
!! This module contains all of Professor Michael Powells original
!! BOBYQA-fortran77 code as private routines.
!! Their declarations are somewhat adapted for fortran95 and
!! the objective function CALFUN is made an argument to the subroutines.
!! This way BOBYQA can easily be called to minimize several different
!! objective functions within the same main program, if necessary.
!! The routine minimize_with_bobyqa is a fortran95 wrapper-routine
!! to include intent(in), (out), (inout) checks and to
!! take care of the allocation of the work-array needed for the f77-routines.
!! Porting done by Erich Wolfgang Steiner. Licence that of Professor
!! Michael Powell, Lesser Gnu Public Licence.

MODULE bobyqa_module

   private :: BOBYQA, BOBYQB, RESCUE, PRELIM, ALTMOV, TRSBOX, UPDATE
...
!!       The search begins at X at entry. X
...
!!     way that satisfies the bounds given in XL and XU.
   ...
   SUBROUTINE minimize_with_bobyqa( N,X,XL,XU,NPT, &
                                    RHOBEG,RHOEND,IPRINT, &
                                    MAXFUN, CALFUN )
      integer, intent(in) :: N
      real(8), intent(inout) :: X(N)
      real(8), intent(in) :: XL(N), XU(N)
      integer, intent(in) :: NPT 
      real(8), intent(in) :: RHOBEG,RHOEND
      ...
      k = (NPT+5)*(NPT+N)+3*N*(N+5)/2
      
      allocate( W(k) )
      
      call BOBYQA( N,NPT,X,XL,XU,RHOBEG,RHOEND,IPRINT, &
                                      MAXFUN,W, CALFUN )
                                      
      deallocate( W )
      
   END SUBROUTINE minimize_with_bobyqa
...
!     Make the call of BOBYQB.
   ...
   SUBROUTINE BOBYQB( N,NPT,X,XL,XU,RHOBEG,RHOEND,IPRINT, &
        ...
        SL,SU,XNEW,XALT,D,VLAG,W,CALFUN)
      IMPLICIT REAL*8 (A-H,O-Z)
      DIMENSION X(*),XBASE(*),XPT(NPT,*),FVAL(*), &
...
!     that do not depend on ZMAT. VLAG is used temporarily for working space.
   ...
   90 IF (DSQ .LE. 1.0D-3*XOPTSQ) THEN
          ...
          SUME=-HALF*XOPTSQ
          DO 100 I=1,N
  100     SUME=SUME+XPT(K,I)*XOPT(I)
          W(NPT+K)=SUME
          TEMP=FRACSQ-HALF*SUME
          DO 110 I=1,N
          W(I)=BMAT(K,I)
          VLAG(I)=SUME*XPT(K,I)+TEMP*XOPT(I)
          IP=NPT+I
          DO 110 J=1,I
...
!     Then the revisions of BMAT that depend on ZMAT are calculated.
  ...
  120     SUMW=SUMW+VLAG(K)
          DO 140 J=1,N
          SUME=(FRACSQ*SUMZ-HALF*SUMW)*XOPT(J)
          DO 130 K=1,NPT
  130     SUME=SUME+VLAG(K)*XPT(K,J)
          W(J)=SUME
          DO 140 K=1,NPT
  140     BMAT(K,J)=BMAT(K,J)+SUME*ZMAT(K,JJ)
          DO 150 I=1,N
          IP=I+NPT
          TEMP=W(I)
          DO 150 J=1,I
...
!     use when VQUAD is calculated.
  ...
  250 W(NPT+K)=SUMA
      ...
      SUME=ZERO
      DO 260 K=1,NPT
  260 SUME=SUME+ZMAT(K,JJ)*W(K)
      BETA=BETA-SUME*SUME
      DO 270 K=1,NPT
  270 VLAG(K)=VLAG(K)+SUME*ZMAT(K,JJ)
      DSQ=ZERO
      BSUM=ZERO
      DX=ZERO
      DO 300 J=1,N
      ...
      SUME=ZERO
      DO 280 K=1,NPT
  280 SUME=SUME+W(K)*BMAT(K,J)
      BSUM=BSUM+SUME*D(J)
      JP=NPT+J
      DO 290 I=1,N
  290 SUME=SUME+BMAT(JP,I)*D(I)
      VLAG(JP)=SUME
      BSUM=BSUM+SUME*D(J)
...
!     into VLAG(NPT+I), I=1,2,...,N.
  ...
  570     W(K)=ZERO
          ...
          SUME=ZERO
          DO 580 K=1,NPT
  580     SUME=SUME+ZMAT(K,J)*VLAG(K)
          DO 590 K=1,NPT
  590     W(K)=W(K)+SUME*ZMAT(K,J)
          DO 610 K=1,NPT
          SUME=ZERO
          DO 600 J=1,N
  600     SUME=SUME+XPT(K,J)*XOPT(J)
          W(K+NPT)=W(K)
  610     W(K)=SUME*W(K)
          GQSQ=ZERO
          GISQ=ZERO
...
!       to the best point so far.
  ...
  650 KNEW=0
      ...
      SUME=ZERO
      DO 660 J=1,N
  660 SUME=SUME+(XPT(K,J)-XOPT(J))**2
      IF (SUME .GT. DISTSQ) THEN
          KNEW=K
          DISTSQ=SUME
      END IF
  670 CONTINUE
...
!     Form the W-vector of the chosen original interpolation point.
  ...
  140 W(NPT+J)=XPT(KNEW,J)
      ...
      ELSE IF (PTSID(K) .EQ. ZERO) THEN
          DO 150 J=1,N
  150     SUME=SUME+W(NPT+J)*XPT(K,J)
      ELSE
          IP=PTSID(K)
          IF (IP .GT. 0) SUME=W(NPT+IP)*PTSAUX(1,IP)
          IQ=DFLOAT(NP)*PTSID(K)-DFLOAT(IP*NP)
          IF (IQ .GT. 0) THEN
              IW=1
              IF (IP .EQ. 0) IW=2
              SUME=SUME+W(NPT+IQ)*PTSAUX(IW,IQ)
          END IF
      END IF
  160 W(K)=HALF*SUME*SUME
...
!     XPT(KNEW,.) is reinstated in the set of interpolation points.
      ...
      SUME=ZERO
      DO 170 J=1,N
  170 SUME=SUME+BMAT(K,J)*W(NPT+J)
  180 VLAG(K)=SUME
      BETA=ZERO
      DO 200 J=1,NPTM
      SUME=ZERO
      DO 190 K=1,NPT
  190 SUME=SUME+ZMAT(K,J)*W(K)
      BETA=BETA-SUME*SUME
      DO 200 K=1,NPT
  200 VLAG(K)=VLAG(K)+SUME*ZMAT(K,J)
      BSUM=ZERO
      DISTSQ=ZERO
      DO 230 J=1,N
      SUME=ZERO
      DO 210 K=1,NPT
  210 SUME=SUME+BMAT(K,J)*W(K)
      JP=J+NPT
      BSUM=BSUM+SUME*W(JP)
      DO 220 IP=NPT+1,NDIM
  220 SUME=SUME+BMAT(IP,J)*W(IP)
      BSUM=BSUM+SUME*W(JP)
      VLAG(JP)=SUME
...
!     all the new interpolation points are included in the model.
  ...
  310 GOPT(I)=GOPT(I)+DIFF*BMAT(KPT,I)
      ...
      SUME=ZERO
      DO 320 J=1,NPTM
  320 SUME=SUME+ZMAT(K,J)*ZMAT(KPT,J)
      TEMP=DIFF*SUME
  ...
  350 RETURN
   ...
   SUBROUTINE PRELIM(N,NPT,X,XL,XU,RHOBEG,IPRINT,MAXFUN,XBASE, &
      ...
      IMPLICIT REAL*8 (A-H,O-Z)
      DIMENSION X(*),XBASE(*),XPT(NPT,*),FVAL(*),GOPT(*), &
...
!     the square of this function.
  ...
  160 CURV=CURV+HCOL(K)*TEMP*TEMP
      ...
      IF (CURV .GT. -GW .AND. CURV .LT. -CONST*GW) THEN
          SCAL=-GW/CURV
          DO 170 I=1,N
          TEMP=XOPT(I)+SCAL*W(I)
  170     XALT(I)=DMAX1(SL(I),DMIN1(SU(I),TEMP))
          CAUCHY=(HALF*GW*SCAL)**2
      ELSE
